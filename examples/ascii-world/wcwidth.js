// Minimal wcwidth implementation (MIT licensed, derived from https://github.com/timoxley/wcwidth.js)
// Handles BMP + surrogate pairs with East Asian Width and emoji wide defaults.
...
const combining = [
  ...
  [0x108D, 0x108D], [0x109D, 0x109D], [0x135D, 0x135F], [0x1712, 0x1714], [0x1732, 0x1734],
  ...
  [0x1A65, 0x1A6C], [0x1A73, 0x1A7C], [0x1A7F, 0x1A7F], [0x1AB0, 0x1ACE], [0x1B00, 0x1B03],
  ...
  [0x1CF8, 0x1CF9], [0x1DC0, 0x1DFF], [0x20D0, 0x20F0], [0x2CEF, 0x2CF1], [0x2D7F, 0x2D7F],
  [0x2DE0, 0x2DFF], [0x302A, 0x302D], [0x302E, 0x302F], [0x3099, 0x309A], [0xA66F, 0xA672],
  ...
  [0xAA35, 0xAA36], [0xAA43, 0xAA43], [0xAA4C, 0xAA4D], [0xAA7C, 0xAA7C], [0xAAB0, 0xAAB0],
  ...
  [0x10AE5, 0x10AE6], [0x10D24, 0x10D27], [0x10F46, 0x10F50], [0x11001, 0x11001], [0x11038, 0x11046],
...
const isCombining = code => combining.some(([a, b]) => code >= a && code <= b);

export function wcwidth(ucs) {
  const code = typeof ucs === 'string' ? ucs.codePointAt(0) : ucs;
  // C0/C1 control characters
  if (code === 0) return 0;
  if (code < 32 || (code >= 0x7f && code < 0xa0)) return 0;
  if (isCombining(code)) return 0;
  // Wide East Asian / emoji default to width 2
  if (
    (code >= 0x1100 &&
      (code <= 0x115f || // Hangul Jamo init. consonants
        code === 0x2329 ||
        code === 0x232a ||
